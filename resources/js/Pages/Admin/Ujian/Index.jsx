...
import { AdminExamHeader, AdminExamTabs, Metric, StatusBadge, fieldClassName } from '@/Components/Features/AdminExam/AdminExamUI';

export default function Index({ exam_packages = [], levels = [] }) {
        ...
        <AuthenticatedLayout>
            ...
            <div className="min-h-screen bg-slate-50 px-4 py-7 dark:bg-[#0b1121] sm:px-6 lg:px-8">
                <div className="mx-auto max-w-7xl space-y-6">
                    <AdminExamHeader eyebrow="Domain ujian mandiri" title="Kelola Ujian" description="Susun paket latihan dan simulasi JLPT tanpa mencampurnya dengan Evaluasi Week." action={<Link href={route('admin.exams.create')} className="inline-flex h-10 items-center gap-2 rounded-md bg-brand-600 px-4 text-sm font-black text-white hover:bg-brand-700"><AddRoundedIcon fontSize="small" /> Buat ujian</Link>} />
                    <AdminExamTabs />
                    ...
                    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                        ...
                        <Metric label="Total pengerjaan" value={exam_packages.reduce((total, item) => total + item.attempt_count, 0)} detail="Attempt final" />
                    </div>
                    ...
                    <section className="border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                        <div className="grid gap-3 border-b border-gray-200 p-4 dark:border-gray-800 md:grid-cols-[1fr_160px_160px]">
                            ...
                            <select value={level} onChange={(event) => setLevel(event.target.value)} className={`${fieldClassName} mt-0`}><option value="all">Semua level</option>{levels.map((item) => <option key={item}>{item}</option>)}</select>
                            ...
                            <table className="w-full min-w-[880px] text-left text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                    {filtered.map((exam) => <tr key={exam.id} className="hover:bg-gray-50/70 dark:hover:bg-gray-800/30">
                                        ...
                                        <td className="px-4 py-4"><div className="flex justify-end gap-1"><Link href={route('admin.exams.preview', exam.slug)} className="grid h-9 w-9 place-items-center rounded-md text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-800 dark:hover:text-white" title="Preview sebagai peserta"><VisibilityOutlinedIcon fontSize="small" /></Link><Link href={route('admin.exams.edit', exam.slug)} className="grid h-9 w-9 place-items-center rounded-md text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-800 dark:hover:text-white" title="Edit paket"><EditOutlinedIcon fontSize="small" /></Link></div></td>
                                    </tr>)}
